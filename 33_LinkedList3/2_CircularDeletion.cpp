...
void deleteNode(Node* &tail , int value){
    // agr list empty hai to kuch delete hi nahi kar sakte
    if (tail == NULL)
    {
        cout << "List is empty" << endl;
    }
        ...
        // non-empty waale case
        // assuming that "value" is present in the LL
        Node* prev = tail; // ek prev banaya jo tail ko point kar ra
        Node* curr = prev -> next; // curr hamesha prev ke ek aage hota hai

        while (curr -> data != value)
        {
            // tb tk dono ko aage badhate jao
            prev = curr;
            curr = curr -> next;
        }
        prev -> next = curr -> next;

        // case if sirf 1 node present ho
        if (curr == prev)
        {
            tail = NULL;
        }
        ...
        // case for >= 2 node
        if (tail == curr) // agr tail curr ko point kar raha hota to tail ko mai prev pe daal deta, cuz curr to delete hone waala hai
        {
            tail = prev;
        }
...
int main(){
    ...
    // deletion
    // ye LL hai : 3 5 6 7 9 10, isme se mai 3 delete karna chah ra
    deleteNode(tail, 3); // 3 delete kar re, 1st element
    print(tail);

    deleteNode(tail, 10); // 10 delete kar re, last element
    print(tail);
